...
export async function captureVitals({ onUpdate = () => {}, signal, fetchImpl = fetch, pollMs = 350, timeoutMs = CAPTURE_TIMEOUT_MS } = {}) {
  const started = await responseBody(await fetchImpl("/api/vitals/session", { method: "POST", signal }));
  if (!started.session_id) throw new Error("The vitals sensor did not start correctly.");
  const deadline = Date.now() + timeoutMs;

  while (Date.now() < deadline) {
    await wait(pollMs, signal);
    const state = await responseBody(await fetchImpl(`/api/vitals/session/${encodeURIComponent(started.session_id)}`, { signal }));
    onUpdate(state);
