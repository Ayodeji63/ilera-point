...
- Unstable or unavailable sensor captures remain recoverable: each screen can be retried or skipped independently, and trustworthy partial results may continue to clinician review.
...
- Result capabilities and readable return codes are hashed at rest and expire after seven days by default. A returning patient also verifies the complete phone number; rate-limited repeat checks remain possible while a clinician is delayed. Retention deadlines cover video, call audio, operational audit, approved benchmarks, and clinical records.
