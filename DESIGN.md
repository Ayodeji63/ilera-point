...
Vitals use two distinct 1152px route boards: pulse/oxygen first and forehead temperature second. Each board stays idle until its own large yellow Start control is pressed. At the large breakpoint, a dark physical-guidance panel and live sensor feedback sit left while readiness, progress, results, and recovery actions sit on white at right. Yoruba-English and Igbo-English guidance uses the patient's chosen language both visibly and through Sahara, with a replay control. The second board acknowledges the completed pulse stage without showing both positioning tasks at once.
...
Vitals is a brief, optional route stage after recording consent and before the first voice question. The joined board now makes the physical order unmistakable: one relaxed finger covers the pulse sensor first; only after that stage finishes does the patient remove the finger and position their forehead 2–5 cm from the temperature sensor. A two-stop route list marks the active and completed sensor, the live sunlit-yellow waveform appears only during pulse acquisition, and the temperature stage replaces it with a steady-position instruction rather than a decorative graph.

The white state surface uses an assertive stage-specific heading, plain-language support, overall progress, and a textual pulse-attempt or temperature-stage status. On mobile this entire surface, including **Skip these checks**, remains before the route list so capture status and recovery stay visible without scrolling. Full success shows temperature, heart rate, and pulse quality. If only one sensor produces a trustworthy result, the surface names that partial result, explains what was unavailable, and offers both **Continue to health questions** and **Retry both checks**. A total failure offers the clinic-green retry and keeps **Continue without vitals** as a full-size pale-green alternative.
